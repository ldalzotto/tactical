...
const flagsSignature = JSON.stringify({ mode, unity, tests, coverage, assertions, debugSymbols });
...
run('cmake', ['-S', '.', '-B', BUILD_DIR, `-DCMAKE_BUILD_TYPE=${mode}`, `-DAPP_UNITY_BUILD=${unity ? 'ON' : 'OFF'}`, `-DAPP_BUILD_TESTS=${tests ? 'ON' : 'OFF'}`, `-DAPP_COVERAGE=${coverage ? 'ON' : 'OFF'}`, `-DAPP_ASSERTIONS=${assertions ? 'ON' : 'OFF'}`, `-DAPP_DEBUG_SYMBOLS=${debugSymbols ? 'ON' : 'OFF'}`]);
