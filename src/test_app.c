...
// Test-only host import (web/wasm-shared.js): queues an input event for the
// next input_poll, else the wasm test runner always polls zero events and
// app_on_next_frame's shift!=0 rebase path goes uncovered.
...
PRIVATE void test_app_init_deinit_and_frames(linear_allocator_t *allocator) {
    // app_init() bootstraps its own allocator from heap_base(); the test
    // allocator is only used to recover the wasm memory budget.
    // app_deinit() pops back to the same watermark for the final
    // linear_allocator_deinit.
    uint32_t memory_size = (uint32_t)(allocator->data.end - allocator->data.begin);
    app_state_t *state = app_init(memory_size, 1000);
    assert_test(state != 0);

    // (0,0) is empty in the default scenario: no-op click, exercises the
    // shift==0 side of the rebase guard.
    ...
    // 15ms since last_frame_ms: below the 16ms interval, so app_on_next_frame
    // asks the host to wait instead of rendering.
    uint32_t wait_ms = app_on_next_frame(state, 1000 + 15);
    assert_test(wait_ms != 0);

    // Click active entity's own tile (p1 at (1,2)): enters movement mode,
    // growing game->scratch and exercising the shift!=0 rebase path.
    int px, py;
    grid_to_screen(state->game.viewport, 1, 2, &px, &py);
    test_push_input_event(state->window, INPUT_EVENT_MOUSE_CLICK, px + 1, py + 1);

    // Exactly 16ms: frame due. Runs input_poll, event loop (with the queued
    // click above), render_frame, and present_window.
    wait_ms = app_on_next_frame(state, 1000 + 16);
    assert_test(wait_ms == 0);
    assert_test(state->game.mode == GAME_MODE_MOVEMENT);

    // 20ms since last render: also above the interval.
