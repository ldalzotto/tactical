...
PUBLIC bool skill_target_in_range(linear_allocator_t *allocator, grid_t grid, slice_entity_t entities, entity_t* attacker, entity_t* target) {
    pathing_state_t pathing = pathing_compute_distances(allocator, grid, entities, target, attacker->position, attacker->skill.range);

    int distance = pathing_distance_at(pathing, grid, target->position);

    pathing_deinit(allocator, pathing);

    return distance >= 0 && distance <= attacker->skill.range;
}
