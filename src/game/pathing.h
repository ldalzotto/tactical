...
//
// mark_occupied_reachable: when false (movement/legality callers), a tile
// occupied by a non-excluded entity gets no distance at all -- unreachable,
// full stop. When true (attack-range display), that tile still gets a
// distance (d+1 from whichever neighbor reached it first) so it shows up as
// reachable-for-targeting, but it is NOT enqueued into the frontier, so the
// BFS never expands past it -- the occupant still occludes tiles behind it,
// it just also gets to be its own destination.
PUBLIC pathing_state_t pathing_compute_distances(linear_allocator_t *allocator, grid_t grid, slice_entity_t entities, entity_t* excluded, position_t from, int max_steps, bool mark_occupied_reachable);
