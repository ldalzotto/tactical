...
// Split out of onNextFrame so tests can exercise the event loop with a
// non-empty slice (the wasm test runner otherwise always polls zero events).
void app_dispatch_input_events(game_state_t *game, linear_allocator_t *allocator, slice_input_event_t events);
