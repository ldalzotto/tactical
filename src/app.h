...
// Dispatches polled input events through the game API. Split out of
// app_on_next_frame so tests can drive the event loop with a non-empty
// slice (the wasm test runner otherwise always polls zero). Returns the
// byte shift from scratch growth (0 if none); caller must rebase its copy.
